  ...
  return (
    <TransitionComponent>
      <nav className="w-screen h-24 flex items-center pr-6 md:px-4 lg:px-10 fixed top-0 z-50 justify-between xl:justify-around">
        <Link
          to="/"
          className=" text-md sm:text-lg lg:text-2xl tracking-wider font-extrabold cursor-pointer flex items-center md:gap-4"
        >
        ...
        <div className="xl:flex gap-24 hidden">
          {Links.map((LinkItem) => (
            <ul key={LinkItem.name} className="relative">
              ...
              >
                <Link className="hover:opacity-60" to={LinkItem.url}>
                  {LinkItem.name}
                </Link>
        ...
        >
          <ul className="flex flex-col text-lg">
            {Links.map((LinkItem) => (
              <li
                key={LinkItem.name}
                className="p-3 hover:opacity-80 cursor-pointer"
              >
                {LinkItem.name === "Services" ? (
                  <div>
                    <div
